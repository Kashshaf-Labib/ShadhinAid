...
import { createPatient, getPatients, searchPatientsByHospital, updatePatient } from "@/utils/queries/patient";
...
const GET = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    await dbConnect();
    if (!hospital_name) {
      const data = await getPatients(type as string, page ? +page : 1, limit ? +limit : 50);
